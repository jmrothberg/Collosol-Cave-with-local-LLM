...
# - Optional: diffusion_client.py and diffusion server for remote image generation
...
# Import diffusion client (will fallback to local if server not available)
USE_DIFFUSION_SERVER = False  # Default value
try:
    from diffusion_client import ImageGenerator as ServerImageGenerator
    USE_DIFFUSION_SERVER = True
    print("[init] Diffusion server client available")
except ImportError as e:
    USE_DIFFUSION_SERVER = False
    print(f"[init] Diffusion server client not available, using local generation: {e}")
...
class ImageGenerator:
    ...
    def _lazy_init(self) -> None:
        ...
        # Special case: Z-Image-Turbo - use the working approach
        if self.model_id == "Z-Image-Turbo" or "Z-Image-Turbo" in self.model_id:
            ...
            try:
                ...
                # Enable Flash Attention (this works in your test code)
                self._pipeline.transformer.set_attention_backend("flash")

                # Compile for speed (this works in your test code)
                self._pipeline.transformer.compile()

                print("Z-Image pipeline loaded successfully!")
                return
            except Exception as e:
                print(f"[image] Failed to load Z-Image pipeline: {e}")
                self._pipeline = None
                return
            ...
            if model_local_path:
                ...
                if "SDXL" in base_name_upper:
                    self._pipeline = StableDiffusionXLPipeline.from_pretrained(
                        ...
                        variant="fp16" if self.device.startswith("cuda") and self._dtype == torch.float16 else None,
    ...
    def generate(self, prompt: str) -> Optional[str]:
        ...
        if self._pipeline is None:
            return None
        try:
            # Special handling for Z-Image-Turbo
            if self.model_id == "Z-Image-Turbo" or "Z-Image-Turbo" in self.model_id:
            ...
            elif isinstance(self._pipeline, FluxPipeline):
                image = self._pipeline(
                    prompt,
                ...
                # Use 10 inference steps for non-FLUX models
            ...
            try:
                ig_path = os.path.join(IN_GAME_IMG_DIR, filename)
                shutil.copyfile(filepath, ig_path)
            except Exception as e:
...
def check_diffusion_server_running(host: str = "127.0.0.1", port: int = 8000) -> bool:
    ...
    try:
        ...
        print(f"Diffusion server not running or not accessible: {e}")
        return False


def start_diffusion_server(gpu_id: int = None):
    """Start the diffusion server if it's not running. Auto-selects GPU intelligently."""
    import subprocess
    import time

    if check_diffusion_server_running():
        print("[init] Diffusion server already running")
        return True
    ...
    if gpu_id is None:
        try:
            import torch
            gpu_count = torch.cuda.device_count()
            print(f"[init] GPU detection: {gpu_count} GPUs available")
            if gpu_count > 1:
                # Multiple GPUs: use GPU 1 to avoid LLM conflicts
                selected_gpu = 1
                print(f"[init] Multiple GPUs detected ({gpu_count}), starting diffusion server on GPU 1")
            else:
                # Single GPU (DGX or Mac): use GPU 0 for everything
                selected_gpu = 0
                print(f"[init] Single GPU detected ({gpu_count} total), diffusion server sharing GPU 0 with LLM")
        except ImportError as e:
            # torch not available, assume single GPU
            selected_gpu = 0
            print(f"[init] GPU detection unavailable ({e}), starting diffusion server on GPU 0")
    else:
        selected_gpu = gpu_id
        try:
            gpu_count = torch.cuda.device_count() if 'torch' in globals() else 1
        except:
            gpu_count = 1
        if gpu_id == 0 and gpu_count == 1:
            print(f"[init] Starting diffusion server on GPU {selected_gpu} (sharing with LLM)")
        else:
            print(f"[init] Starting diffusion server on specified GPU {selected_gpu}")

    print("[init] Starting diffusion server...")
    try:
        # Build command with GPU parameter
        cmd = ["./start_diffusion_server.sh", "--gpu", str(selected_gpu)]
        ...
        if check_diffusion_server_running():
            print("[init] Diffusion server started successfully")
            return True
        else:
            print("[init] Failed to start diffusion server")
            return False

    except Exception as e:
        print(f"[init] Error starting diffusion server: {e}")
        return False
...
def launch_gradio_ui(state_mgr: StateManager, llm: LLMEngine, image_gen: Optional[ImageGenerator]) -> None:
    ...
    def do_reload_diffuser(diffuser_id: str):
        ...
        if diffuser_id:
            # Use server-based ImageGenerator if available, otherwise local
            if USE_DIFFUSION_SERVER:
                # Check if diffusion server is running, start it if not
                if not check_diffusion_server_running():
                    if not start_diffusion_server():
                        _append_debug("[reload] Failed to start diffusion server, using local")
                        image_gen = ImageGenerator(model_id=diffuser_id, device_preference=None, local_root=diffusion_models_root())
                    else:
                        try:
                            image_gen = ServerImageGenerator(model_id=diffuser_id)
                        except Exception as e:
                            _append_debug(f"[reload] Server connection failed: {e}, using local")
                            image_gen = ImageGenerator(model_id=diffuser_id, device_preference=None, local_root=diffusion_models_root())
                else:
                    try:
                        image_gen = ServerImageGenerator(model_id=diffuser_id)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Use server-based ImageGenerator if available, otherwise local (defaults to Z-Image)
    if USE_DIFFUSION_SERVER:
        # Check if diffusion server is running, start it if not
        if not check_diffusion_server_running():
            print("[init] Diffusion server not running, attempting to start...")
            if not start_diffusion_server():
                print("[init] Failed to start diffusion server, falling back to local generation")
                USE_DIFFUSION_SERVER = False
                image_gen = ImageGenerator()
            else:
                try:
                    image_gen = ServerImageGenerator(model_id="Z-Image-Turbo")
                    print("[init] Connected to DiffusionD server with model: Z-Image-Turbo")
                except Exception as e:
                    print(f"[init] Failed to connect to diffusion server: {e}")
                    print("[init] Falling back to local generation")
                    USE_DIFFUSION_SERVER = False
                    image_gen = ImageGenerator()
        else:
            try:
